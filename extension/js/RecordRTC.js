...
function RecordRTC(mediaStream, config) {
  ...
  function initRecorder(initCallback) {
    ...
    const Recorder = new GetRecorderType(mediaStream, config);
  ...
  function getDataURL(callback, _mediaRecorder) {
    ...
    const blob = _mediaRecorder ? _mediaRecorder.blob : (mediaRecorder || {}).blob;
    ...
    if (typeof Worker !== 'undefined' && !navigator.mozGetUserMedia) {
      const webWorker = processInWebWorker(readFile);
  ...
  const WARNING = `It seems that recorder is destroyed or "startRecording" is not invoked for ${config.type} recorder.`;
  ...
  var returnObject = {
    ...
    /**
				 ...
				 * });
				 */
    setRecordingDuration(recordingDuration, callback) {
    ...
    setAdvertisementArray(arrayOfWebPImages) {
						   ...
						   const { length } = arrayOfWebPImages;
	 ...
	 */
    destroy() {
			 const disableLogsCache = config.disableLogs;
...
RecordRTC.getFromDisk = function (type, callback) {
  ...
  DiskStorage.Fetch((dataURL, _type) => {
...
RecordRTC.writeToDisk = function (options) {
  ...
  if (options.audio && options.video && options.gif) {
    options.audio.getDataURL((audioDataURL) => {
      options.video.getDataURL((videoDataURL) => {
        options.gif.getDataURL((gifDataURL) => {
          DiskStorage.Store({
  ...
  this.startRecording = function () {
    ...
    if (mediaType.video) {
      ...
      if (isMediaRecorderCompatible() && !!mediaType.audio && typeof mediaType.audio === 'function') {
        const videoTrack = getTracks(mediaStream, 'video')[0];
    ...
    if (!!mediaType.audio && !!mediaType.video) {
      ...
      let isSingleRecorder = isMediaRecorderCompatible() === true;
      ...
      if (isSingleRecorder === true) {
        ...
        self.videoRecorder.initRecorder(() => {
          self.audioRecorder.initRecorder(() => {
 ...
 */
  this.stopRecording = function (callback) {
    ...
    if (this.audioRecorder) {
      this.audioRecorder.stopRecording((blobURL) => {
        callback(blobURL, 'audio');
      });
    }

    if (this.videoRecorder) {
      this.videoRecorder.stopRecording((blobURL) => {
        callback(blobURL, 'video');
      });
    }

    if (this.gifRecorder) {
      this.gifRecorder.stopRecording((blobURL) => {
  ...
  this.getDataURL = function (callback) {
    this.getBlob((blob) => {
      if (blob.audio && blob.video) {
        getDataURL(blob.audio, (_audioDataURL) => {
          getDataURL(blob.video, (_videoDataURL) => {
            callback({
              audio: _audioDataURL,
              video: _videoDataURL,
            });
    ...
    function getDataURL(blob, callback00) {
      if (typeof Worker !== 'undefined') {
        const webWorker = processInWebWorker((_blob) => {
          postMessage(new FileReaderSync().readAsDataURL(_blob));
        });
...
function getTracks(stream, kind) {
  ...
  return stream.getTracks().filter((t) => t.kind === (kind || 'audio'));
}
      ...
      if (typeof config.timeSlice === 'number') {
        if (e.data && e.data.size) {
          ...
          if (typeof config.ondataavailable === 'function') {
            ...
            const blob = config.getNativeBlob ? e.data : new Blob([e.data], {
 ...
 */
  ...
  let context = RecordRTC.Storage.AudioContextConstructor;

  // creates an audio node from the microphone incoming stream
  let audioInput = context.createMediaStreamSource(mediaStream);

  const legalBufferValues = [0, 256, 512, 1024, 2048, 4096, 8192, 16384];
 ...
 */
  ...
  function onAudioProcessDataAvailable(e) {
    ...
    const chLeft = new Float32Array(left);
...
function CanvasRecorder(htmlElement, config) {
  ...
  const _isChrome = (!!window.webkitRTCPeerConnection || !!window.webkitGetUserMedia) && !!window.chrome;

  let chromeVersion = 50;
  const matchArray = navigator.userAgent.match(/Chrom(e|ium)\/([0-9]+)\./);
  ...
  this.record = function () {
    ...
    if (isCanvasSupportsStreamCapturing && !config.useWhammyRecorder) {
      ...
      try {
        const mdStream = new MediaStream();
  ...
  this.getWebPImages = function (callback) {
    ...
    const framesLength = whammy.frames.length;
    whammy.frames.forEach((frame, idx) => {
      const framesRemaining = framesLength - idx;
      if (!config.disableLogs) {
      ...
      const webp = frame.image.toDataURL('image/webp', 1);
 ...
 */
  ...
  function dropBlackFrames(_frames, _framesToCheck, _pixTolerance, _frameTolerance, callback) {
    ...
    asyncLoop({
      ...
      functionToLoop(loop, f) {
        let matchPixCount; let endPixCheck; let
          maxPixCount;

        const finishImage = function () {
          ...
          image.onload = function () {
            ...
            for (let pix = 0; pix < endPixCheck; pix += 4) {
              ...
              const colorDifference = Math.sqrt(
                (currentColor.r - sampleColor.r) ** 2
				+ (currentColor.g - sampleColor.g) ** 2
				+ (currentColor.b - sampleColor.b) ** 2,
              );
 ...
 */
  this.stop = function (callback) {
    ...
    setTimeout(() => {
      ...
      dropBlackFrames(whammy.frames, -1, null, null, (frames) => {
...
var Whammy = (function () {
  ...
  function whammyInWebWorker(frames) {
    ...
    function getStrLength(string, offset) {
      return parseInt(string.substr(offset + 4, 4).split('').map((i) => {
    ...
    const webm = new ArrayToWebM(frames.map((frame) => {
...
var DiskStorage = {
  ...
  init() {
		  ...
		  const dbVersion = 1;
		  const dbName = this.dbName || location.href.replace(/\/|:|#|%|\.|\[|\]/g, '');
			  let db;
		  ...
		  function putInDB() {
			  const transaction = db.transaction([self.dataStoreName], 'readwrite');
...
function GifRecorder(mediaStream, config) {
  ...
  const isHTMLObject = mediaStream instanceof CanvasRenderingContext2D || mediaStream instanceof HTMLCanvasElement;
...
function MultiStreamsMixer(arrayOfMediaStreams, elementClass) {
  ...
  function getMixedStream() {
    ...
    let fullcanvas;
    arrayOfMediaStreams.forEach((stream) => {
  ...
  function getMixedVideoStream() {
    ...
    capturedStream.getTracks().filter((t) => t.kind === 'video').forEach((track) => {
  ...
  function getMixedAudioStream() {
    ...
    self.audioSources.forEach((audioSource) => {
  ...
  this.appendStreams = function (streams) {
    ...
    streams.forEach((stream) => {
      ...
      if (stream.getTracks().filter((t) => t.kind === 'video').length) {
        const video = getVideo(stream);
        video.stream = stream;
        videos.push(video);

        newStream.addTrack(stream.getTracks().filter((t) => t.kind === 'video')[0]);
      }
      ...
      if (stream.getTracks().filter((t) => t.kind === 'audio').length) {
        ...
        newStream.addTrack(self.audioDestination.stream.getTracks().filter((t) => t.kind === 'audio')[0]);
      }
  ...
  this.releaseStreams = function () {
    ...
    if (self.audioSources.length) {
      self.audioSources.forEach((source) => {
        source.disconnect();
      });
      self.audioSources = [];
    }
...
if (typeof RecordRTC === 'undefined') {
  ...
  if (typeof define === 'function' && define.amd) {
    define('MultiStreamsMixer', [], () => MultiStreamsMixer);
  }
}
...
function MultiStreamRecorder(arrayOfMediaStreams, options) {
  ...
  function getAllVideoTracks() {
    const tracks = [];
    arrayOfMediaStreams.forEach((stream) => {
      getTracks(stream, 'video').forEach((track) => {
        tracks.push(track);
      });
    });
    return tracks;
...
function RecordRTCPromisesHandler(mediaStream, options) {
  ...
  this.stopRecording = function () {
    return new Promise((resolve, reject) => {
      try {
        self.recordRTC.stopRecording((url) => {
  ...
  this.getDataURL = function (callback) {
    return new Promise((resolve, reject) => {
      try {
        self.recordRTC.getDataURL((dataURL) => {
          resolve(dataURL);
        });
      } catch (e) {
        reject(e);
